...
from database.models import User, MenuItem, Order, OrderItem, UserRole, OrderStatus
...
def get_or_create_user(db, username, email, password, role):
    ...
    if user:
        return user
    return create_user(db, username, email, password, role)
...
def populate_database():
    ...
    for username, email, password, role in users:
        try:
            user = get_or_create_user(db, username, email, password, role)
    ...
    # Create menu items (first clear existing ones)
    ...
    for name, desc, price, category, image in menu_items:
        try:
            ...
            print(f"Error creating menu item {name}: {e}")
            db.rollback()
